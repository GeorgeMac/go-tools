...
var AtResult interface{}

func makeTypes() []types.Type {
	var fields []*types.Var
	typs := []types.Type{tStr, tPStr1, tInt, tChanInt1}
	var tags []string
	availTags := []string{"", "short", "average length", "a really quite long tag"}
	for i := 0; i < 20; i++ {
		fields = append(fields, types.NewVar(0, nil, fmt.Sprintf("field%d", i), typs[i%len(typs)]))
		tags = append(tags, availTags[i%len(availTags)])
	}
	return []types.Type{
		tStr,
		tPStr1,
...
func TestMap(t *testing.T) {
	...
	if l := len(tmap.table); l != 0 {
		t.Errorf("Map is empty but has buckets")
	}
}
